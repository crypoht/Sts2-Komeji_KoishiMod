...
using System;
using BaseLib.Utils;
using MegaCrit.Sts2.Core.Localization.DynamicVars; // 🌟 必须引入动态变量命名空间
...
namespace KomeijiKoishi.Cards.Fumo
{
    ...
    public sealed class OkinaFumo_Koishi : CustomCardModel
    {
        ...
        protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
        {
            ...
            if (maxSelect > 0 && exhaustPileCards.Count > 0)
            {
                maxSelect = Math.Min(maxSelect, exhaustPileCards.Count);

                CardSelectorPrefs prefs = new CardSelectorPrefs(base.SelectionScreenPrompt, 0, maxSelect);
                
                IEnumerable<CardModel> selectedCards = await CardSelectCmd.FromSimpleGrid(choiceContext, exhaustPileCards, player, prefs);
